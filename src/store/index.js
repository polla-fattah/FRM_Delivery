...
const store = new Vuex.Store({
    ...
    actions: {
        ...
        async loadPosition({ commit }, payload) {
            if (payload.isMobile) {
                Geolocation.getCurrentPosition().then(position => { commit("setPosition", position) })
                Geolocation.watchPosition({}, (position, err) => { commit("setPosition", position) })
            } else {
                navigator.geolocation.getCurrentPosition(position => { commit("setPosition", position) });
                navigator.geolocation.watchPosition(position => { commit("setPosition", position) });
            }
        }
    }
})
